# mess-attendance-prediction
A Google-powered system that predicts daily mess attendance to reduce food wastage and improve campus sustainability.

## Problem
In college messes, a large amount of food is wasted daily due to inaccurate estimation of student turnout.

## Solution
This project predicts daily mess attendance using past data and day-based factors such as weekends, holidays and meal prepared.  
Based on the prediction, it recommends how much food should be prepared to reduce wastage.

## Google Tech. used
- Google Forms (data collection)
- Google Sheets (data storage & analysis)
- Google Apps Script (prediction logic & automation)
- Google Charts (visualization)

## How It Works
1. Mess manager fills a daily Google Form.
2. Data is stored automatically in Google Sheets.
3. Apps Script processes historical data.
4. System predicts required meal count.
5. Output is shown in Sheets as text and charts.

## Impact
- Reduces food wastage
- Saves cost
- Improves campus sustainability

## Hackathon
Hack'Vento 2k26 – GDG on Campus IET DAVV, Indore


